<template>
  <div class="Products">
    <h1 class="Products__title">Мобильное приложение dozp.kz</h1>
    <div class="custom_border" />
    <div class="welcome_img">
      <div class="app">
        <img src="../assets/img/products/app.png" />
        <div class="app__links">
          <img src="../assets/img/products/apple.png" />
          <img src="../assets/img/products/market.png" />
        </div>
      </div>
      <img src="../assets/img/products/circles.png" />
    </div>
    <div class="advantages">
      <h2>преимущества использования dozp.kz</h2>
      <div class="advantages__grid">
        <div class="advantages__grid__single">
          <h3>Нет аналогов</h3>
          <div class="grid_border" />
          <p>
            Уникальный fintech-продукт <br />
            Иновационный на мировом рынке
          </p>
        </div>
        <div class="advantages__grid__single">
          <h3>
            новое решение в <br />
            маркетинге
          </h3>
          <div class="grid_border" />
          <p>
            100% эффективный таргетинг <br />
            Качественные клиенты
          </p>
        </div>
        <div class="advantages__grid__single">
          <h3>Выгодно для всех</h3>
          <div class="grid_border" />
          <p>
            МФО - Клиент - ТЦ,ТРЦ
          </p>
        </div>
      </div>
    </div>
    <div class="sup_txt">
      <div class="sup_txt__content">
        <div class="image_wrapper">
          <img src="../assets/img/products/time.png" />
        </div>
        <p>
          Надежно распознает, верифицирует, оценивает потенциального заёмщика и
          выдает деньги,
          <span>меньше чем за 1 минуту ПЕРВЫЙ ЗАЁМ</span> и за
          <span>30 секунд последующие</span>
        </p>
        <div class="image_wrapper">
          <img src="../assets/img/products/coins.png" />
        </div>
      </div>
    </div>
    <div class="terminal">
      <h1>
        Терминалы выдачи займов kreditomat
      </h1>
      <div class="custom_border" />
      <img class="terminal__image" src="../assets/img/products/terminal.png" />
    </div>
    <div class="term_adv">
      <h2>Преимущества использования терминала</h2>
      <div class="term_adv__grid">
        <div class="term_adv__grid__single">
          <img src="../assets/img/products/eye.png" />
          <p>
            Наши терминалы «НА-ВИДУ» в торговых центрах, метро и наиболее
            проходимых местах
          </p>
        </div>
        <div class="term_adv__grid__single">
          <img src="../assets/img/products/shop_hand.png" />
          <p>
            Сами места стимулируют клиента на заем и совершение с помощью него
            ИМПУЛЬСИВНОЙ ПОКУПКИ
          </p>
        </div>
        <div class="term_adv__grid__single">
          <img src="../assets/img/products/money_hand.png" />
          <p>
            НЕ ТРЕБУЕТСЯ НИКАКОЙ РЕКЛАМЫ, один терминал выдает до 100 кредитов в
            сутки
          </p>
        </div>
        <div class="term_adv__grid__single">
          <img src="../assets/img/products/tv.png" />
          <p>
            Всеми процессами в терминале управляет уникальное программное
            обеспечение
          </p>
        </div>
        <div class="term_adv__grid__single">
          <img src="../assets/img/products/criminal.png" />
          <p>
            Отсутствие кредитных менеджеров и мошенничества
          </p>
        </div>
        <div class="term_adv__grid__single">
          <img src="../assets/img/products/debit_cart.png" />
          <p>
            Нет комиссий за обналичивание и возврат
          </p>
        </div>
      </div>
    </div>
    <div class="dbl_terminal">
      <img src="../assets/img/products/dbl_terminal.png" />
      <div class="dbl_terminal__txt">
        <h2>требования к локации под терминал</h2>
        <div class="cus_brd" />
        <p>Охрана</p>
        <p>Наличие розетки</p>
        <p>Наличие видеонаблюдения</p>
        <p>Доступность резервного питания</p>
        <p>Возможность подключения мобильного интернета (хороший сигнал)</p>
      </div>
    </div>
    <div class="atm_txt">
      <div class="atm_txt__content">
        <img src="../assets/img/products/atm.png" />
        <div>
          <p>
            - Размещайте терминалы в местах с высокой проходимостью
          </p>
          <p>
            - Размещайте терминалы у главного входа или возле супермаркетов
            (большие продуктовые магазины внутри ТЦ, ТРЦ)
          </p>
          <p>
            - Также, Вы можете разместить терминал в зоне банкоматов. <br />
            &nbsp; Обычно, таких зон в ТЦ, ТРЦ несколько, выберите самую
            популярную (обычно возле супермаркета)
          </p>
        </div>
      </div>
    </div>
    <div class="click_adv">
      <h1>
        Мобильное приложение click advance
      </h1>
      <div class="custom_border" />
      <div class="welcome_img">
        <div class="app">
          <img src="../assets/img/products/app2.png" />
          <div class="app__links">
            <img src="../assets/img/products/apple.png" />
            <img src="../assets/img/products/market.png" />
          </div>
        </div>
        <img src="../assets/img/products/steps.png" />
      </div>
    </div>
    <div class="adv_click">
      <h2>
        преимущества использования click advance
      </h2>
      <div class="adv_click__grid">
        <div class="adv_click__grid__single">
          <img src="../assets/img/products/handshake.png" />
          <p>
            Возврат кредитов гарантирует компания-работодатель на основании
            подписанного договора.
          </p>
        </div>
        <div class="adv_click__grid__single">
          <img src="../assets/img/products/piechart.png" />
          <p>
            Мы можем предложить клиентам низкие и выгодные процентные ставки по
            сравнению с другими кредитными продуктами.
          </p>
        </div>
        <div class="adv_click__grid__single">
          <img src="../assets/img/products/regression.png" />
          <p>
            Поскольку уровень неработающих кредитов очень низкий, персонал по
            сбору платежей и связанные с этим расходы минимальны.
          </p>
        </div>
        <div class="adv_click__grid__single">
          <img src="../assets/img/products/money_talk.png" />
          <p>
            Таким образом, мы предлагаем удобный продукт и получаем 45-50%
            постоянных клиентов, которые постоянно им пользуются.
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style lang="scss" scoped>
@import "../assets/style/fonts.scss";
.Products {
  padding-top: 130px;
  &__title {
    //@include medium;
    font-size: 40px;
    line-height: 47px;
    text-align: center;
    text-transform: uppercase;
    color: #623f99;
  }
}
.custom_border {
  width: 160px;
  border-bottom: 2px solid #623f99;
  margin: 10px auto 60px;
}
.welcome_img {
  display: flex;
  align-items: flex-start;
  width: 1200px;
  max-width: 100%;
  margin: 0 auto;
  .app {
    display: flex;
    flex-direction: column;
    align-items: center;
    margin-right: 52px;
    &__links {
      margin-top: 22px;
      img:first-of-type {
        margin-right: 20px;
      }
    }
  }
}
.advantages {
  padding: 60px 0;
  h2 {
    //@include medium;
    font-size: 24px;
    line-height: 28px;
    text-align: center;
    text-transform: uppercase;
    color: #d5dd25;
    margin-bottom: 30px;
  }
  &__grid {
    display: grid;
    grid-template-columns: repeat(3, 387px);
    justify-content: space-between;
    width: 1200px;
    max-width: 100%;
    margin: 0 auto;
    &__single {
      height: 174px;
      background: #ffffff;
      box-shadow: 0px 4px 20px rgba(146, 115, 194, 0.5);
      border-radius: 4px;
      text-align: center;
      padding-top: 50px;
      &:nth-child(2) {
        padding-top: 27px;
      }
      h3 {
        //@include medium;
        font-size: 24px;
        line-height: 28px;
        text-transform: uppercase;
        color: #623f99;
      }
      .grid_border {
        width: 70px;
        border-bottom: 1px solid #623f99;
        margin: 10px auto;
      }
      p {
        @include normal;
        font-size: 16px;
        line-height: 19px;
        color: #131313;
      }
    }
  }
}
.sup_txt {
  background: #f8f2ff;
  height: 140px;
  padding-top: 20px;
  &__content {
    width: 1200px;
    max-width: 100%;
    margin: 0 auto;
    display: flex;
    align-items: center;
    justify-content: space-between;
    p {
      @include normal;
      font-size: 16px;
      line-height: 19px;
      text-align: center;
      text-transform: uppercase;
      color: rgba(19, 19, 19, 0.5);
      max-width: 851px;
      span {
        color: #623f99;
      }
    }
  }
  .image_wrapper {
    width: 110px;
    display: flex;
    justify-content: flex-start;
    align-items: center;
    &:last-of-type {
      justify-content: flex-end !important;
    }
  }
}
.terminal {
  padding-top: 120px;
  padding-bottom: 60px;
  text-align: center;
  h1 {
    //@include medium;
    font-size: 40px;
    line-height: 47px;
    text-transform: uppercase;
    color: #623f99;
  }
}
.term_adv {
  padding-bottom: 60px;
  h2 {
    //@include medium;
    font-size: 24px;
    line-height: 28px;
    text-align: center;
    text-transform: uppercase;
    color: #d5dd25;
  }
  &__grid {
    display: grid;
    grid-template-columns: repeat(3, 387px);
    justify-content: space-between;
    grid-row-gap: 20px;
    width: 1200px;
    max-width: 100%;
    margin: 0 auto;
    padding-top: 30px;
    &__single {
      height: 237px;
      background: #ffffff;
      box-shadow: 0px 4px 20px rgba(146, 115, 194, 0.5);
      border-radius: 8px;
      padding: 30px;
      text-align: center;
      p {
        @include normal;
        font-size: 16px;
        line-height: 19px;
        color: #131313;
        margin-top: 27px;
      }
    }
  }
}
.dbl_terminal {
  display: flex;
  justify-content: center;
  align-items: flex-start;
  padding-bottom: 60px;
  img {
    margin-right: 79px;
  }
  &__txt {
    h2 {
      //@include medium;
      font-size: 24px;
      line-height: 28px;
      text-transform: uppercase;
      color: #623f99;
    }
    .cus_brd {
      width: 160px;
      border-bottom: 2px solid #623f99;
      margin-top: 10px;
      margin-bottom: 38px;
    }
    p {
      position: relative;
      @include normal;
      font-size: 18px;
      line-height: 21px;
      color: #131313;
      margin-bottom: 21px;
      margin-left: 39px;
      &::after {
        content: url("../assets/img/main/checkbox.png");
        position: absolute;
        top: 2px;
        left: -39px;
      }
      &:last-of-type {
        margin-bottom: 0;
      }
    }
  }
}
.atm_txt {
  background: #f8f2ff;
  height: 156px;
  padding-top: 30px;
  &__content {
    width: 1200px;
    max-width: 100%;
    margin: 0 auto;
    display: flex;
    align-items: center;
    img {
      margin-right: 60px;
    }
    p {
      @include normal;
      font-size: 16px;
      line-height: 19px;
      text-transform: uppercase;
      color: #838383;
      &:nth-child(2) {
        margin: 10px 0;
      }
    }
  }
}
.click_adv {
  padding-top: 120px;
  padding-bottom: 60px;
  h1 {
    //@include medium;
    font-size: 40px;
    line-height: 47px;
    text-align: center;
    text-transform: uppercase;
    color: #623f99;
  }
}
.adv_click {
  padding-bottom: 120px;
  h2 {
    //@include medium;
    font-size: 24px;
    line-height: 28px;
    text-align: center;
    text-transform: uppercase;
    color: #d5dd25;
    margin-bottom: 30px;
  }
  &__grid {
    display: grid;
    grid-template-columns: repeat(4, 285px);
    justify-content: space-between;
    width: 1200px;
    max-width: 100%;
    margin: 0 auto;
    &__single {
      background: #ffffff;
      box-shadow: 0px 4px 20px rgba(146, 115, 194, 0.5);
      border-radius: 4px;
      height: 255px;
      padding: 30px 20px 20px;
      text-align: center;
      p {
        @include normal;
        font-size: 16px;
        line-height: 19px;
        color: #131313;
      }
      img {
        margin-bottom: 20px;
      }
    }
  }
}
</style>
